...
func Attack():
	get_node("AnimationPlayer").play("Stab_E")	
			
func death():
	get_node("Sprite/EnemyBody/CollisionShape2D").set_deferred("disabled", true)
	get_node("AnimationPlayer").play("Death_E")
...
func _on_EnemyBody_area_entered(area):
	if area.name=="SwordBlade":
		...
		player.stream = load("res://Audio/Sounds/Game/345436__artmasterrich__male-hurt-04.ogg")
		player.play()
		if current_hp <= 0:
			state = "Death"
